...
import { ChartRequestBody } from "../../../types/prisma"
...
export default async function index(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "PUT") {
    try {
      ...
      if (performance) {
        const update = prisma.performance.updateMany({
        ...
        const weekPerformance = prisma.weekPerformance.create({
          data: {
        ...
        const performanceId = await prisma.performance.create({
          data: {
        ...
        await prisma.weekPerformance.create({
          data: {
